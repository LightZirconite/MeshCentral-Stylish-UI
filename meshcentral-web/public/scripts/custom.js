...
(function () {
    ...
    function waitForSubmenu(cb) {
        const maxTries = 30;
        let tries = 0;
        (function poll() {
            const el = document.getElementById('uiSubmenu');
            if (el) cb(el);
            else if (++tries < maxTries) setTimeout(poll, 300);
        })();
    }

    function addSwitchThemeItem(submenu) {
        if (submenu.querySelector('.mc-switch-theme')) return;

        // Get translated label
        const referenceLink = document.querySelector('#p2AccountActions a[onclick="return account_showThemesSwitcher()"]');
        const label = referenceLink ? referenceLink.textContent.trim() : 'Switch Theme';

        const themeItem = document.createElement('div');
        themeItem.className = 'userDropdownMenuItem mc-switch-theme';

        themeItem.innerHTML = `
            <svg class="svg-inline--fa fa-palette userDropdownMenuIcon"
                 viewBox="0 0 640 640"
        ...
        themeItem.onclick = function (e) {
            e.stopPropagation();
            if (typeof account_showThemesSwitcher === 'function') {
                account_showThemesSwitcher();
            }
        };

        submenu.insertBefore(themeItem, submenu.firstChild);
    }
...
(function () {
    ...
    // --- 6) Delete offline devices in one click, confirm only live devices ---
    let armedDeleteNodeId = null;

    function getNodeById(nodeid) {
        try {
            if (typeof window.getNodeFromId === 'function') return window.getNodeFromId(nodeid);
        } catch (_) {}
        if (window.currentNode && window.currentNode._id === nodeid) return window.currentNode;
        return null;
    }
    ...
    function isNodeConnected(nodeid) {
        ...
        if (typeof node.conn === 'number') return node.conn !== 0;
        if (typeof node.conn === 'string') return node.conn !== '' && node.conn !== '0';
        return node.conn === true || node.connected === true;
    }

    function deleteNodeWithLiveGuard(nodeid, executeDelete) {
        ...
        if (window.confirm('L’appareil "' + name + '" est connecté. Supprimer quand même ?')) run();
        return false;
    }
    ...
    function patchDeviceDeleteConfirmation() {
        if (typeof window.p10showDeleteNodeDialog === 'function' && !window.p10showDeleteNodeDialog.__mcWrapped) {
            const wrappedDialog = function (nodeid) {
                ...
                return deleteNodeWithLiveGuard(id, function () {
